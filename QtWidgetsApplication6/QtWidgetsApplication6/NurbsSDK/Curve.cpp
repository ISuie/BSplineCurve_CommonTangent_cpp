...
void Curve::evalSecDers(std::vector<double>& u, std::vector<double>& basisValue)
{
	double tol = 1e-15;
	int p = m_Basis[0].getDegree();
	int uPt = m_Basis[0].getKnotSize() - 1 - p - 1;
	...
	int uind;
	double w = 0.0, fac, fac2, dNdxi = 0.0, dNdxi2 = 0.0;
	for (int i = 0; i <= p; i++)
	{
		uind = uspan - p + i;
		w += Nu[i] * weight[uind];
		dNdxi += Nu_ders[1][i] * weight[uind];
		dNdxi2 += Nu_ders[2][i] * weight[uind];
	}
	for (int i = 0; i <= p; i++)
	{
		uind = uspan - p + i;
		fac2 = weight[i] / (w * w * w);
		basisValue[i] = (Nu_ders[2][i] * w * w - dNdxi2 * Nu[i] * w - 2 * Nu_ders[1][i] * dNdxi * w + 2 * dNdxi * dNdxi * Nu[i]) * fac2;
	}
}